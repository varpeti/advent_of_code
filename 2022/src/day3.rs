...
pub fn day3_b(input: &String) -> String {
    ...
    loop { // iter.next_chunk() is experimental, so I use this instead.
        let first = rucksacks.next();
        if let None = first { // guard for the unwraps, only None if no rucksack left
            break;
        } 
        let mut common = first.unwrap();
        let group = [rucksacks.next().unwrap(), rucksacks.next().unwrap()];
        for i in 0..(group.len()) {
            common = common.intersection(&group[i]).map(|r| *r).collect();
        }
