...
fn calculate_score(input: &str, move_map: &HashMap<&str, HashMap<&str, u32>>) -> u32 {
    input
        .split('\n')
        .map(|line| {
            if line.is_empty() {
...
pub fn day2_a(input: &str) -> String {
    ...
    move_map.insert(
        "A",
        HashMap::from([("X", 1 + 3), ("Y", 2 + 6), ("Z", 3)]),
    );
    move_map.insert(
        "B",
        HashMap::from([("X", 1), ("Y", 2 + 3), ("Z", 3 + 6)]),
    );
    move_map.insert(
        "C",
        HashMap::from([("X", 1 + 6), ("Y", 2), ("Z", 3 + 3)]),
    );
